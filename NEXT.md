...
2. **アニメーション追加（座る等）** ← ✅完了（2026-07-09、下記「アニメーション実装」参照）
...
**座り中の腕の位置バグ修正【✅完了・2026-07-09】**
座った時に腕が変な位置（横に大きく開く／後ろに流れる）に行く不具合を修正。
- 原因1: Mixamoの座りクリップは`upperArm`/`lowerArm`だけでなく**肩(shoulder/鎖骨)ボーン**も動かしていた。procedural側は肩を触っていなかったため、肩が上がったまま→腕全体が横に流れて見えていた。`isSitting`中は`lShoulder`/`rShoulder`を`(0,0,0)`にリセットするよう追加
- 原因2（デバッグで判明）: Playgroundでのテスト中、`isWalking`が真のまま収束せず、if/elseで腕の分岐が常に「歩行中」側（何もしない＝歩行クリップの腕振りがそのまま出る）に取られていた。`APPROACH_LERP=0.02`の収束待ちがヘッドレスブラウザの低fps（実測17〜18fps程度）で想定より遅かっただけで、実機の60fps環境ではもっと速いはず。デバッグ用に一時的に状態をconsole.logして原因を特定→削除済み
- `SIT_ARM`/`SIT_ELBOW`の角度を実際にPlaywright+スクリーンショットで目視しながら試行錯誤で調整（`z`と`x`の効果が直感と違い相互作用するため理論より実測が早かった）。最終値: `SIT_ARM = { z: 1.15, x: 0.55 }`、`SIT_ELBOW = { z: 0.55 }`。腕以外（脚・座高オフセット等）は今回変更していない
