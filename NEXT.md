...
1. **本番VRMモデルへの差し替え** ← 未着手・最優先
   - `public/avatar/sample.vrm` がまだ three-vrm 公式サンプルのまま。差し替えるだけで見た目のインパクトが一番大きい
   - VRoid Studioで自作 → `public/avatar/` に配置して渡してもらえば実装は差し替えのみ
   - `surprised` 表情もこのタイミングでVRoidStudioに追加しておく
