...
2. **アニメーション追加（座る等）** ← 最優先・進行中（下記「アニメーション実装」参照）
   - 歩行・発話ジェスチャー・部屋徘徊・接近/後退は実装済み。次は座りモーションなど
...
### 座るアニメーション【次にやる・最優先】
2026-07-08〜。歩行系（歩く・徘徊・接近/後退）が一通り完成したので、次はアニメーションの幅を広げる方向で座りモーションに着手する。
- `room.glb`に椅子がすでにあるので、徘徊のゴールとして「椅子に座って一息つく」を追加すると、待機中の生活感演出（`idle.vrma`のアイデアとほぼ同じ狙い）にもそのまま繋がる
- やり方は歩行と同じ想定: Mixamoで座りモーション（Sitting Idle等、In Place）→ Blender MCPでVRoidモデルにリターゲット → VRMA書き出し → `Avatar.tsx`に統合
- 歩行の時に踏んだ落とし穴（World/Local空間の使い分け、ボーン全選択エクスポート、spineの絶対代入）は同じ手順を流用できるはず
- 未着手。他にもクリーンアップ・リハーサル等のタスクは残っているが、いまはアニメーション側を優先して進める
