...
- 対策: `isWalking`分岐に、`walkWeight`が低いほど強く・高いほど弱く基本姿勢へ引っ張る補正
  (`standPull = max(0, 1 - walkWeight) * 0.6`)を追加。通常の歩行スイングを妨げないよう、
  weight→1に近づくと寄与はほぼ0になる設計。Playwrightで座る→立つを再現し、修正前後のスクリーンショットで
  改善を確認（完全に一瞬の乱れが無くなるわけではないが、水平近くまで伸びる大きな破綻は解消）
