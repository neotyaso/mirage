...
2. **アニメーション追加（座る等）** ← 大枠は完了・微調整は保留中（下記「アニメーション実装」参照）
   - 歩行・発話ジェスチャー・部屋徘徊・接近/後退・座る（椅子まで歩く→着席→起立）まで実装済み
   - 座り中の腕の位置は本人の実機で未解決のまま保留（詳細は下記「座り中の腕の位置バグ修正」）。次にBlender/実機を触れるタイミングでまとめて詰める
...
**座り中の腕の位置バグ修正【⚠️一部対応・2026-07-09、本人環境で未解決】**
座った時に腕が変な位置（横に大きく開く／後ろに流れる）に行く不具合。
- 原因1: Mixamoの座りクリップは`upperArm`/`lowerArm`だけでなく**肩(shoulder/鎖骨)ボーン**も動かしていた。procedural側は肩を触っていなかったため、肩が上がったまま→腕全体が横に流れて見えていた。`isSitting`中は`lShoulder`/`rShoulder`を`(0,0,0)`にリセットするよう追加（これ自体は要因の一つとして妥当なはず）
- 原因2（デバッグで判明）: Playgroundでのテスト中、`isWalking`が真のまま収束せず、if/elseで腕の分岐が常に「歩行中」側（何もしない＝歩行クリップの腕振りがそのまま出る）に取られていた。`APPROACH_LERP=0.02`の収束待ちがヘッドレスブラウザの低fps（実測17〜18fps程度）で想定より遅かっただけで、実機の60fps環境ではもっと速いはず、という前提で対応
- `SIT_ARM`/`SIT_ELBOW`の角度をPlaywright+スクリーンショットで目視しながら調整（`z: 1.15, x: 0.55` / elbow `z: 0.55`）。Claude側のPlaywright検証では毎回意図通り（腕が前・膝側）に見えたが、**本人が実機（実ブラウザ）で確認したところ直っていなかった**との報告あり。Playwrightでの見た目確認とユーザー実機での見た目に齟齬がある状態で、原因未特定のまま保留
- 疑うべき点（次回再開時のメモ）: ブラウザキャッシュ／別ブランチ／`playground.html`でなく本番`App.tsx`で見ていて自動着席条件を満たしていなかった、等の切り分けは口頭では試したが実際に本人の画面を見て確認できていない。実機でconsole.logや実際の値を見ながら再度切り分けるのが確実
...
7. **目が合ってるかゲーム**【✅完了・2026-07-09】
   - MediaPipe FaceLandmarkerは追加ライブラリなしで478点（基本468点+虹彩10点）を常時返すことが判明（Tasks APIはlegacyのFaceMeshと違い`refineLandmarks`指定不要）。虹彩中心(468/473)と目の両端(33/133・362/263)から視線の水平比率を`useFaceDetection.ts`の`gazeRatioRef`として算出
