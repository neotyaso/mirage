...
### 歩行アニメーション【✅一次実装完了・要調整・2026-07-06】

Mixamo「Walking」FBX（In Placeチェック済み）→ Blender MCP経由でVRoidモデルにリターゲット → `public/avatar/walk.vrma`として書き出し → `Avatar.tsx`に統合、まで完了。
...
3. blender-mcp経由でVRoidモデル＋MixamoFBXを同一シーンに読み込み
4. VRM1.0の`vrm_addon_extension.vrm1.humanoid.human_bones`から標準ボーン名を取得（推測でなく実データから対応表を作成）
5. **腰・両脚の9ボーンだけ**Copy Rotation（World空間）でリターゲット。腕はMixamoのAポーズとVRMのTポーズがズレるため今回は対象外にし、手続き型コード側に委譲
6. フレーム1・8・16・24を目視確認（サイクル通しで破綻なし）
7. `nla.bake`でMixamo依存を切り離し、`export_scene.vrma`でVRMA書き出し
8. `Avatar.tsx`に`AnimationMixer`で統合。`isWalking`（接近度が目標に追いついてない間）でウェイトをフェード、歩行中は脚クリップの位相に同期した腕の対角振りを追加

**既知の課題（明日やること）**
- 腰の前後の傾きが脚と一緒に上半身へ伝わり、歩行中に体が後ろに反って見える → 見た目の違和感の主因。スペイン/胸のボーンで軽くカウンター回転をかけるか、腰のピッチ成分だけ弱めるなどの調整が必要
- 腕の対角振りはまだ「それっぽい」レベルの近似（脚の実際の位相と厳密に同期していない）。実機で見ながら振り幅・タイミングを追い込む
- 後退（来場者が離れる時）は同じ歩行クリップを流用してないので棒立ちで戻る＝見た目のチェックは未実施
- 腰の上下バウンス（自然な体重移動の上下動）は今回スコープ外（回転のみリターゲット、位置は未対応）
- Mac側での実機確認（カメラ込みの人物検知→歩行トリガーの一連の流れ）

| ファイル | 用途 | 状態 |
|---------|------|------|
| `walk.vrma` | 来場者検知→歩いてくる | ✅実装済み・微調整待ち |
