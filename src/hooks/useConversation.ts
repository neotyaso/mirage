...
【行動タグ】相槌を打ちたい時だけ、文頭に[nod]（頷く）または[tilt]（首をかしげる）を付けてよい（付けなくてもよい。多用しない）。タグは読み上げられず動きに変換されるので、その後の文はタグなしと同じ自然な文で続ける。

例:「え、マジ？ガハハ！それ最高すぎる」「うわ〜センスいいじゃ〜ん！今日は誰と来たの？」「[nod]わかるわかる！それめっちゃ良いよね」`;
...
// "stretch"/"wave"/"drink"/"armsCrossed"はLLMには使わせず手動トリガー専用のため、
// 型には含めるがACTION_TAGS(LLM検出対象)には含めない
export type ActionTag = "nod" | "tilt" | "stretch" | "wave" | "drink" | "armsCrossed";
