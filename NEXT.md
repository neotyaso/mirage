...
### 歩行アニメーション【未着手・方針決定済み】
1. Mixamoから歩行モーションをFBXでダウンロード（Adobeアカウントで無料、クレジット表記不要）
2. BlenderでVRM標準ヒューマノイドボーンにリターゲット（blender-mcp経由でClaudeがある程度スクリプト可能。ボーン向きのズレ等は視覚確認しながらの試行錯誤になる見込み）
3. VRM Animationアドオンで`.vrma`として書き出し
4. **重要**: VRM標準ヒューマノイドボーンにリターゲットしておけば、そのVRMAファイルは他のVRMモデル（男性キャラ等）にもそのまま流用可能。モデルごとにやり直す必要はない
5. アプリ側で`AnimationMixer`再生。下半身（脚・腰）はクリップ担当、上半身（腕・視線・呼吸）は既存の手続き型コードが担当、と役割分担して競合を避ける
6. 今の`vrm.scene.position.z`のZ移動（接近演出）と歩行クリップを同時に動かし「歩きながら近づく」動きにする

| ファイル | 用途 | 状態 |
|---------|------|------|
| `walk.vrma` | 来場者検知→歩いてくる | 方針決定・ファイル未取得 |
